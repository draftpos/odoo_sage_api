...
class HavanoSagePoller(models.AbstractModel):
    ...
    def cron_poll_sage_changes(self):
        ...
        try:
            ...
            for change in changes:
                ...
                # Fallbacks for new C# API structure
                entity_type = change.get('entityType') or change.get('entity') or change.get('Entity') or ''
                ...
                action = change.get('action') or change.get('Action') or 'update'
                action = action.lower()
                
                sage_id_val = change.get('entityId') or change.get('externalReference') or change.get('ExternalReference') or ''
                sage_id = str(sage_id_val)
    ...
    def _sync_product(self, sage_id, data, action):
        ...
        # Map selling price from sellingPrices list (priceList == configured name) or fallback fields
        price_list_name = self.env['ir.config_parameter'].sudo().get_param('havano_sage_sync.price_list_name', default='Price List 1')
        prices = data.get('sellingPrices', [])
        if prices:
            # Check for exact match with price_list_name or fallback to 1 / "1"
            price_match = next((p for p in prices if str(p.get('priceList')) == str(price_list_name) or str(p.get('priceList')) == '1'), None)
            if price_match:
                vals['list_price'] = price_match.get('priceExcl', 0.0)
        else:
